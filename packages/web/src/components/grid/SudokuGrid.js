...
export default function SudokuGrid({ gridSeat, id }) {
    ...
    const wipingSet = new Set();
    for (const w of wipingCells) {
        if (w.targetSeat === gridSeat)
            wipingSet.add(`${w.row}-${w.col}`);
    }
    // Completion ripple — triggered when my own cell becomes correct
    useEffect(() => {
        if (!isMe || !lastCorrectCell || !myGrid || !mySolution)
            return;
    ...
    const handleMouseMove = useCallback((e) => {
        ...
        if (isNaN(row) || isNaN(col))
            return;
        if (lastSentCursor.current?.row === row && lastSentCursor.current?.col === col)
            return;
    ...
    for (let r = 0; r < 9; r++) {
        for (let c = 0; c < 9; c++) {
            ...
            const isHighlight = isMe && selectedCell != null && !isSelected && (r === selectedCell.row || c === selectedCell.col ||
            ...
            cells.push(_jsx(SudokuCell, { row: r, col: c, value: val, isGiven: isMe ? isGiven : undefined, isCorrect: isMe ? isCorrect : undefined, isDanger: isMe ? isDanger : undefined, isSelected: isSelected, isHighlight: isHighlight, isSameNumber: isSameNumber, isWiping: isWiping, isCompletionFlash: isCompletionFlash, isOpponentGiven: isOpponentGiven, isOpponentFilled: isOpponentFilled, isOpponentCursor: isOpponentCursor, isBoxTop: r > 0 && r % 3 === 0, isBoxLeft: c > 0 && c % 3 === 0, isPreBoxRight: c === 2 || c === 5, isPreBoxBottom: r === 2 || r === 5, isReadonly: !isMe, onPointerDown: isMe ? handlePointerDown : undefined }, key));
        }
    }
    return (_jsx("div", { id: id, className: `sudoku-grid${isMe ? ' is-me' : ''}`, onMouseMove: isMe ? handleMouseMove : undefined, children: cells }));
}
