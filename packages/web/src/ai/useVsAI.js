...
import { generatePuzzle, handleCellInput, applyDamageFromAttack, buildCellQueue, scheduleNext, STARTING_HEALTH, } from '@sudoku-fighting/shared';
...
// ---------------------------------------------------------------------------
export let vsAiPlayerMove = null;
// Called by SpLobbyScreen once, then again each round from startVsAIRound
let _startRoundCallback = null;
export function startVsAIRound(roundNumber) {
    _startRoundCallback?.(roundNumber);
}
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeEmptyRoundState() {
    return {
        puzzles: [
...
export function useVsAI() {
    ...
    const roundState = useRef(makeEmptyRoundState());
    const attackTimers = useRef(new Map());
    const botQueue = useRef([]);
    const botSession = useRef({ stopped: true, timeouts: new Set() });
    const localRoundWins = useRef([0, 0]);
    function clearAllTimers() {
        for (const t of attackTimers.current.values())
            clearTimeout(t);
        attackTimers.current.clear();
        for (const t of botSession.current.timeouts)
            clearTimeout(t);
        botSession.current.stopped = true;
        botSession.current.timeouts.clear();
    }
    function dispatchRoundEnd(winnerSeat) {
        clearAllTimers();
        if (winnerSeat !== -1) {
            localRoundWins.current[winnerSeat]++;
        }
        const roundWins = [...localRoundWins.current];
        ...
        if (localRoundWins.current[0] >= 2 || localRoundWins.current[1] >= 2) {
            ...
            setTimeout(() => {
                injectServerMessage({ type: 'match_end', payload: { winnerSeat: matchWinner, winnerName } });
            }, 200);
        }
    }
    function checkRoundEnd() {
        ...
        if (h[0] <= 0 || h[1] <= 0) {
            const winner = h[0] <= 0 && h[1] <= 0 ? -1 : h[0] <= 0 ? 1 : 0;
            dispatchRoundEnd(winner);
            return true;
        }
        return false;
    }
    function scheduleAttack(attackId, delay) {
        const t = setTimeout(() => {
    ...
    function processMove(seat, row, col, value) {
        ...
        for (const ev of events) {
            ...
            if (ev.type === 'puzzle_complete') {
                ...
                setTimeout(() => dispatchRoundEnd(seat), 200);
            }
        }
        checkRoundEnd();
    }
    function startRound(roundNumber) {
        ...
        const difficulty = useGameStore.getState().spDifficulty;
            ...
            for (let r = 0; r < 9; r++) {
                for (let c = 0; c < 9; c++) {
                    if (rs.puzzles[1].playerGrid[r][c] === null && !inQueue.has(`${r}-${c}`)) {
                        ...
                        if (val != null) {
                            queue.unshift({ row: r, col: c, value: val });
                            inQueue.add(`${r}-${c}`);
                        }
                    }
                }
        ...
        const startTimer = setTimeout(() => {
            botSession.current.timeouts.delete(startTimer);
            scheduleNext(botQueue.current, botSession.current, difficulty, (r, c, v) => {
                processMove(1, r, c, v);
            }, replenish);
        }, startDelay);
        botSession.current.timeouts.add(startTimer);
    }
    useEffect(() => {
        if (gameMode !== 'singleplayer' || currentScreen !== 'gameplay')
            return;
