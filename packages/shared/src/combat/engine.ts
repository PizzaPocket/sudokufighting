...
export function handleCellInput(
  ...
  const attackType: AttackType = round.consecutiveCorrect[attackerSeat] % 3 === 0 ? 'kick' : 'punch';
  ...
  // Completions
  const completions: Array<{ kind: 'row' | 'col' | 'box'; index?: number; boxRow?: number; boxCol?: number }> = [];
  if (isRowComplete(puzz, row)) completions.push({ kind: 'row', index: row });
  if (isColComplete(puzz, col)) completions.push({ kind: 'col', index: col });
  const boxRow = Math.floor(row / 3) * 3;
  const boxCol = Math.floor(col / 3) * 3;
  if (isBoxComplete(puzz, boxRow, boxCol)) completions.push({ kind: 'box', boxRow, boxCol });

  let compAttackType: AttackType | null = null;
  let compDmg = 0;
...
export function applyDamageFromAttack(
  round: RoundState,
  attackId: string
): { events: ServerMessage[]; attackType: AttackType; attackerSeat: 0 | 1; defenderSeat: 0 | 1 } | null {
  const idx = round.pendingAttacks.findIndex(a => a.id === attackId);
  if (idx === -1) return null;
  const attack = round.pendingAttacks.splice(idx, 1)[0];
  if (attack.counterUsed) return null;
...
export function applyCounterDamage(
  round: RoundState,
  attackId: string
): { events: ServerMessage[]; counterSeat: 0 | 1 } | null {
  const idx = round.pendingAttacks.findIndex(a => a.id === attackId);
  if (idx === -1) return null;
  const attack = round.pendingAttacks.splice(idx, 1)[0];
